...
const redTeamOverlayPath = resolve(repoRoot, "tasks/2026-08-red-team-findings.yaml");
...
function applyOverlay(base, overlay, sourceName) {
  ...
  result.meta = {
    ...(result.meta || {}),
    overlays: [...new Set([...(result.meta?.overlays || []), sourceName])],
    state_model: "historical-registry + audit-overlays",
  };
  ...
  for (const phase of (overlay.phase_overrides || [])) {
    ...
    if (existing) Object.assign(existing, phase);
      ...
      const created = { id: phase.id, name: phase.name, exit: phase.exit };
      result.phases.push(created);
      phaseById.set(String(phase.id), created);
    }
  }
...
function readRegistryAsJSON() {
  ...
  if (existsSync(auditOverlayPath)) result = applyOverlay(result, readYaml(auditOverlayPath), auditOverlayPath);
  if (existsSync(redTeamOverlayPath)) result = applyOverlay(result, readYaml(redTeamOverlayPath), redTeamOverlayPath);
...
if (existsSync(auditOverlayPath)) watch(auditOverlayPath, { persistent: true }, onRegistryFileEvent);
if (existsSync(redTeamOverlayPath)) watch(redTeamOverlayPath, { persistent: true }, onRegistryFileEvent);

server.listen(PORT, "127.0.0.1", () => {
  console.log(`Dashboard: http://localhost:${PORT}/`);
  console.log(`Watching: ${registryPath}`);
  console.log(`Audit overlay: ${auditOverlayPath}`);
  console.log(`Red-team overlay: ${redTeamOverlayPath}`);
});
