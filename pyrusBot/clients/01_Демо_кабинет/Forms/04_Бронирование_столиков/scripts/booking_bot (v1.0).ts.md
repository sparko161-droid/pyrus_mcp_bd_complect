---
title: "Код: booking_bot (v1.0).ts"
audience: "tech"
pyrus_id: "RIDa5lg8w0J"
pyrus_parent: "VgVzLyMyHkK"
synced_at: "2026-08-11T18:29:01.000Z"
---
...
// ═══════════════════════════════════════════════════════════════════════════
//  БОТ БРОНИРОВАНИЯ СТОЛИКОВ (Форма 2445773)
// ═══════════════════════════════════════════════════════════════════════════
...
type StepType =
  ...
  | "checkmark" | "file" | "time" | "date" | "date_time";
...
const ASK_SEQUENCE: StepDefinition[] = [
  ...
  { code: FIELD_CODES.GUEST_PHONE, question: "Укажите, пожалуйста, Ваш номер телефона для связи.", hint: "Подойдёт номер в формате 79XXXXXXXXX или 89XXXXXXXXX." },
  { code: FIELD_CODES.GUEST_EMAIL, question: "Укажите, пожалуйста, Вашу электронную почту.", hint: "Если не хотите указывать, напишите «нет».", optional: true, type: "email" },
  { code: FIELD_CODES.REQUEST_TYPE, question: "Укажите тип заявки:" },
  { code: FIELD_CODES.EVENT_NAME, question: "Уточните, пожалуйста, наименование мероприятия:" },
  { code: FIELD_CODES.EVENT_NAME_OTHER, question: "Уточните, пожалуйста, название мероприятия:", min: 2, max: 100 },
  { code: FIELD_CODES.GUESTS_COUNT, question: "На какое количество персон планируется бронирование?", min: 1, max: 200, type: "number" },
  { code: FIELD_CODES.HALL_ZONE, question: "Укажите желаемый зал или зону:" },
  { code: FIELD_CODES.SPECIAL_REQUESTS, question: "Есть ли особые пожелания к бронированию?", hint: "Если пожеланий нет, напишите «нет».", optional: true },
  { code: FIELD_CODES.BOOKING_DATE, question: "Укажите желаемую дату и время визита.", hint: "Формат: ДД.ММ.ГГГГ ЧЧ:ММ (например: 25.10.2026 19:00).", type: "date_time" },
];
...
const MSG = {
  ...
  DONE: "Спасибо! Все данные по бронированию собраны. Передаю заявку администратору, с Вами свяжутся для подтверждения.",
...
function parseDateTime(input: string): string | undefined {
  ...
  if (dateMatch) {
    ...
    const date = new Date(Date.UTC(year, month, day, hours - TIMEZONE_OFFSET_HOURS, minutes));
  ...
  if (timeOnly) {
    ...
    const now = new Date();
    const date = new Date(Date.UTC(now.getFullYear(), now.getMonth(), now.getDate(), hours - TIMEZONE_OFFSET_HOURS, minutes));
    return date.toISOString().replace(/\.\d{3}Z$/, "Z");
  }
  return undefined;
}
...
async function parseAnswer(
  ...
  if (!text && !attachments.length) return { failed: true };
  ...
  switch (type) {
    ...
    case "date_time": {
      ...
      return dt ? { value: dt } : { failed: true };
    }
    case "choice": {
...
function resolveStepType(step: StepDefinition, schema: FieldSchema): StepType {
  ...
  switch (schema.type) {
    ...
    case "due_date_time": return "date_time";
    default: return "text";
  }
}
...
export default handleBookingBot;
...
async function run(request: BotHookRequest, task: any, client: PyrusApiClient): Promise<any> {
  ...
  if (trigger.key && trigger.key === state.last_event_key) {
    log("Повторный вызов хука — выходим");
    return null;
  }
  state.last_event_key = trigger.key;

  const textLower = normalize(trigger.text);
  if (OPERATOR_TRIGGERS.some(t => textLower.includes(t))) {
    log(`Обнаружен запрос оператора: "${trigger.text}"`);
    state.finished = true;
    const fieldUpdates: any[] = [{ id: stateFieldId, value: JSON.stringify(state) }];
    const statusFieldId = form.idOf(FIELD_CODES.STATUS);
    if (statusFieldId) fieldUpdates.push({ id: statusFieldId, value: { choice_id: 1 } }); // Новая
    return {
      text: MSG.OPERATOR,
  ...
  // Если был текущий шаг — разбираем ответ гостя
  if (state.step_field_code) {
    ...
    if (currentStep && schema && isFieldVisible(schema, form, values)) {
      const stepType = resolveStepType(currentStep, schema);
      const options = await form.optionsFor(client, schema);
      const parsed = await parseAnswer(
        currentStep, schema, stepType, trigger.text, trigger.attachments, options, client, form
      );

      if (parsed.value !== undefined) {
        log(`Ответ на шаг ${currentStep.code} принят: ${JSON.stringify(parsed.value)}`);
        updates.push({ id: schema.id, value: parsed.value });
        ...
        log(`Шаг ${currentStep.code} пропущен пользователем`);
        state.completed_steps.push(currentStep.code);
        state.error_count = 0;
        state.step_field_code = null;
      } else {
        state.error_count++;
        log(`Не удалось разобрать ответ на шаг ${currentStep.code} (ошибок подряд: ${state.error_count})`);

        if (state.error_count >= MAX_ERRORS) {
          state.finished = true;
          updates.push({ id: stateFieldId, value: JSON.stringify(state) });
          const statusFieldId = form.idOf(FIELD_CODES.STATUS);
          if (statusFieldId) updates.push({ id: statusFieldId, value: { choice_id: 1 } });
          return {
            text: MSG.GIVE_UP,
  ...
  // Подбираем следующий неверный/незаполненный видимый шаг
  let nextStep: StepDefinition | undefined = undefined;
  let nextSchema: FieldSchema | undefined = undefined;

  for (const step of ASK_SEQUENCE) {
    const schema = form.byCode.get(step.code);
    if (!schema) continue;
    if (!isFieldVisible(schema, form, values)) continue;

    const val = values.get(schema.id);
    if (isEmptyValue(val, schema.type) && !state.completed_steps.includes(step.code)) {
      nextStep = step;
      nextSchema = schema;
      break;
    }
  ...
  if (!nextStep || !nextSchema) {
    log("Все видимые шаги пройдены — опрос завершён!");
    state.finished = true;
    updates.push({ id: stateFieldId, value: JSON.stringify(state) });
    const statusFieldId = form.idOf(FIELD_CODES.STATUS);
    if (statusFieldId) updates.push({ id: statusFieldId, value: { choice_id: 1 } }); // Новая
  ...
  const asks = (state.ask_counts[nextStep.code] || 0) + 1;
  state.ask_counts[nextStep.code] = asks;

  if (asks > MAX_ASKS_PER_FIELD) {
    log(`Зацикливание на шаге ${nextStep.code} (${asks} раз) — переводим на человека`);
    state.finished = true;
    updates.push({ id: stateFieldId, value: JSON.stringify(state) });
    return {
      text: MSG.GIVE_UP,
  ...
  if (options.length) {
    const formattedOpts = options.map((opt, i) => `${i + 1}. ${opt}`).join("\n");
    textPrompt += `\n\nВарианты:\n${formattedOpts}`;
...
async function flushLogs(client: PyrusApiClient, taskId: number): Promise<void> {
  ...
  try {
    await client.tasks.addComment(taskId, {
      text: `🤖 [booking_bot log]\n${logs.join("\n")}`,
    });
  } catch (e: any) {
    // Внутренний лог не должен ломать вызов
  }
}
```
